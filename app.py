...
[data-testid="stAppViewBlockContainer"] { padding-top: 0rem !important; padding-bottom: 0rem !important; padding-left: 1rem !important; padding-right: 1rem !important; max-width: 100% !important; margin-top: -5rem !important; overflow-x: hidden; }
...
.nav-pill { background: rgba(255, 255, 255, 0.05); padding: 8px 14px; border-radius: 8px; border: 1px solid rgba(255,255,255,0.08); display: flex; align-items: center; gap: 8px; }
...
def execute_real_hybrid_model_and_backtest(data_df):
    LOOK_BACK = 30
    TEST_DAYS = 7
    ...
    for i in range(TEST_DAYS):
        diff = hybrid_test_preds[i] - actual_test_y[i]
...
# ==========================================
# 4. TAB STATE & DYNAMIC SIDEBAR INPUTS
# ==========================================
st.sidebar.markdown("### ⚙️ SYSTEM PREFERENCES")
user_capital = st.sidebar.number_input("Capital Allocation (USD)", value=13450.00, step=100.0)
user_wallet = st.sidebar.text_input("Wallet Public Key", value="0xBwqw...1248")
...
vol_24h = (live_vol * current_price / 1_000_000) if live_vol else (df['Volume'].iloc[-1] * current_price / 1_000_000)

price_diff = live_pred - current_price
...
<div class="top-nav">
    ...
    <div class="nav-right">
        ...
        <div class="nav-pill" style="color: #e2a8ff;"><span style="color:#8a849b;">💳</span> {user_wallet[:6]}...{user_wallet[-4:]}</div>
        <div class="lang-dropdown-wrapper">
            <div class="lang-btn">🌐 {lang_code.upper()} ▾</div>
        </div>
...
with col_main:
    if is_loading:
        st.markdown(f'''<div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 60vh; width: 100%;"><div style="width: 50px; height: 50px; border: 3px solid rgba(245, 166, 35, 0.2); border-radius: 50%; border-top-color: #f5a623; animation: spin 1s linear infinite;"></div><div style="margin-top:20px; color:#8a849b; font-weight:600; letter-spacing:2px; font-size:0.8rem;">ACCESSING {tab_param.upper()} SECURE NODE...</div></div>''', unsafe_allow_html=True)
    else:
        if tab_param == "Trade":
            ...
            <div class="stat-box"><span class="stat-title">H-V8 Target (T+1)</span><span class="stat-val">${live_pred:,.2f}</span></div>
            ...
            <div class="chart-legend"><span>Base: <span>BTC</span></span> <span>Quote: <span>USDT</span></span> <span>Model: <span>LSTM</span></span> <span style="color:#8a849b">Real Acc: <span>{real_acc:.1f}%</span></span></div>
            """, unsafe_allow_html=True)
            
            plot_df = df.iloc[-90:].copy()
            if live_price is not None: plot_df.loc[pd.Timestamp.now(tz='UTC')] = pd.Series({'Close': current_price})
            
            fig = go.Figure()
            fig.add_trace(go.Scatter(x=plot_df.index, y=plot_df['Close'], mode='lines', line=dict(color='#f5a623', width=2, shape='spline'), fill='tozeroy', fillcolor='rgba(245, 166, 35, 0.05)'))
            fig.add_hline(y=live_pred, line_dash="dash", line_color="#00ff9d" if price_diff > 0 else "#ff4d4d", opacity=0.5)
            ...
            news_html = "".join([f"""<div class="news-row"><div class="news-row-left"><div class="n-source">{art['source']}</div><div class="n-title">{art['title']}</div></div><div class="n-badge {"pos" if art['score'] > 0.1 else "neg" if art['score'] < -0.1 else "neu"}">{art['score']*100:+.1f}%</div></div>""" for art in articles])
...
with col_side:
    ...
    <div class="rp-balances"><div class="rp-bal-col"><span>Capital Allocation</span><span class="rp-bal-val">{format_inr(user_capital * usd_inr_rate)} (${user_capital:,.2f})</span></div><div class="rp-bal-col" style="text-align: right;"><span>Projected Value</span><span class="rp-bal-val {'text-green' if diff_pct > 0 else 'text-red'}">${user_capital * (1 + (diff_pct/100)):,.2f}</span></div></div>
    <div class="rp-input-group"><div class="rp-label-row"><span>Target Execution Price</span></div><div class="rp-input"><span>${live_pred:,.2f}</span><span class="text-max">TARGET</span></div></div>
